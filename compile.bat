...
IF "%ARG1%"=="release" (
	...
	cl /MP /MTd /DEBUG /Zi /EHsc %SOURCE% /I %SDLINC% /I %SDL_TTFINC% /I %SDL_IMGINC% /I %~dp0src\headers\ /link /LIBPATH:%SDLLIB% /LIBPATH:%SDL_TTFLIB% /LIBPATH:%SDL_IMGLIB% SDL2main.lib SDL2.lib SDL2_ttf.lib SDL2_image.lib opengl32.lib /out:%OUTPUT% /SUBSYSTEM:CONSOLE
	
    echo ---- COMPLETED DEBUG BUILD ---- 
)
echo.
REM RELEASE? - probably not optimized
REM echo RELEASE BUILD
REM cl /DFC_USE_SDL_GPU /DNFONT_USE_TTF /EHsc %SOURCE% /I %SDLINC% /I %SDL_GPUINC% /I %SDL_TTFINC% /I %~dp0src\headers\ /link /LIBPATH:%SDLLIB% /LIBPATH:%SDL_GPULIB% /LIBPATH:%SDL_TTFLIB% SDL2main.lib SDL2.lib SDL2_gpu.lib SDL2_ttf.lib opengl32.lib /out:%OUTPUT% /SUBSYSTEM:CONSOLE

REM /I C:\Users\SSYUN\dev\sdl_game\main\aika\lib\SDL\include
REM /LIBPATH:C:\Users\SSYUN\dev\sdl_game\main\aika\lib\SDL\lib\ 
REM SDL2.lib
REM SDL2_gpu.lib opengl32.lib
